 ...
 * @since JDK 1.6
 */
public class ChangeToOneLineFormat implements ActionListener {
	...
	private void identifyDstComponent(Component component)
	{
		if (component == null) {
			...
			try {
				...
				case JSON:
					dstPanel.getTextArea().setText(WanShiWu.jsonStrOnelineFormat(dstPanel.getTextArea().getText()));
					break;
				case XML:
					Document doc = WanShiWu.getDocFromString(dstPanel.getTextArea().getText());
					dstPanel.getTextArea().setText(WanShiWu.getJDom2DocWhitespaceNormalisedString(doc));
					break;
				}
			} catch (Exception e) {
				e.printStackTrace();
